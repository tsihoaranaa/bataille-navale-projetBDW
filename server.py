...
class WebServer(HTTPServer):
    ...
    def url_for(self, name_or_path, filename=None):
        """
        Build URLs for templates with compatibility Flask-style and direct static path.
        - url_for('accueil') -> '/accueil'
        - url_for('static', filename='images/logo-batnav.svg') -> '/<directory>/static/images/logo-batnav.svg'
        """
        # Flask style static file helper
        if name_or_path == 'static':
            if filename is None:
                return ''
            relative = path.normpath(path.join('static', filename))
            return '/' + path.normpath(path.join(SESSION.get('DIRECTORY', ''), relative)).replace('\\', '/')

        # route helper (or direct resource path)
        route = name_or_path.strip('/') if isinstance(name_or_path, str) else ''
        if route == '' or route == SESSION.get('DIRECTORY', ''):
            return '/'
        return '/' + route
